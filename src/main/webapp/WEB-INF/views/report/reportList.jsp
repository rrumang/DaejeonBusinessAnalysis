				...
				<div class="col-sm-12 col-md-12">
					<div class="dataTables_length" id="dataTable_length">
						...
						<button id="csearchBtn" type="button"
							class="btn btn-primary">비교분석</button>
					</div>
				</div>
			</div>
				...
				<tbody>
					...
					<c:forEach var="report" items="${reportList }" varStatus="status">
						<tr class="reportTr">
							...
							<td class="show">
								<a href="${pageContext.request.contextPath}/report/showReport?report_cd=${report.report_cd }&report_kind=${report.report_kind}">
									<img id="icon" src="${pageContext.request.contextPath}/img/report.png"/>
								</a>
							</td>
						</tr>
					</c:forEach>
			...
			<div class="text-center">
				<ul class="pagination">
					<c:choose>
						...
						<c:otherwise>
							<li>
								<a class=page href="${pageContext.request.contextPath}/report/reportList?page=1&pageSize=${pageVo.pageSize}" data-page="1" data-size="${pageVo.pageSize}">&lt;&lt;</a>
								<a class=page href="${pageContext.request.contextPath}/report/reportList?page=${pageVo.page - 1}&pageSize=${pageVo.pageSize}" data-page="${pageVo.page -1}" data-size="${pageVo.pageSize}">&lt;</a>
							</li>
						</c:otherwise>
					</c:choose>
					...
					<c:forEach begin="1" end="${paginationSize }" step="1" var="i">
						<c:choose>
							...
							<c:otherwise>
								<li>
									<a class=page href="${pageContext.request.contextPath}/report/reportList?page=${i}&pageSize=${pageVo.pageSize}"	data-page="${i }" data-size="${pageVo.pageSize }">${i }</a>
								</li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					...
					<c:choose>
						...
						<c:otherwise>
							<li>
								<a class=page href="${pageContext.request.contextPath}/report/reportList?page=${pageVo.page + 1}&pageSize=${pageVo.pageSize}" data-page="${pageVo.page +1}" data-size="${pageVo.pageSize}">&gt;</a>
								<a class=page href="${pageContext.request.contextPath}/report/reportList?page=${paginationSize}&pageSize=${pageVo.pageSize}" data-page="${paginationSize}" data-size="${pageVo.pageSize}">&gt;&gt;</a>
							</li>
						</c:otherwise>
					</c:choose>
